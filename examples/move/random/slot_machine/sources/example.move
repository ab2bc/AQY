...
///
/// Anyone can create a new game for the current epoch by depositing AQY as the initial balance. The creator can
/// withdraw the remaining balance after the epoch is over.
///
/// Anyone can play the game by betting on X AQY. They win X with probability 49% and lose the X AQY otherwise.
///
...
public fun create(reward: Coin<AQY>, ctx: &mut TxContext) {
...
public fun close(game: Game, ctx: &mut TxContext): Coin<AQY> {
...
entry fun play(game: &mut Game, r: &Random, coin: &mut Coin<AQY>, ctx: &mut TxContext) {
    assert!(ctx.epoch() == game.epoch, EInvalidEpoch);
