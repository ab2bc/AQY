...
fun mint(addr: address, amount: u64, scenario: &mut ts::Scenario) {
    transfer::public_transfer(coin::mint_for_testing<AQY>(amount, scenario.ctx()), addr);
...
fun test_game() {
    ...
    let coin = ts.take_from_sender<Coin<AQY>>();
    ...
    let mut coin: Coin<AQY> = ts.take_from_sender();
