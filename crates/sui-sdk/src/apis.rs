...
impl CoinReadApi {
    ...
    /// or use `None` for the default `Coin<AQY>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///
    /// The coins can be filtered by `coin_type` (e.g., 0x168da5bf1f48dafc111b0a488fa454aca95e0b5e::usdc::USDC)
    /// or use `None` to use the default `Coin<AQY>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// it will use the default `Coin<AQY>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
...
impl GovernanceApi {
    ...
    /// Return the latest AQY system state object on-chain, or an error upon failure.
    ///
