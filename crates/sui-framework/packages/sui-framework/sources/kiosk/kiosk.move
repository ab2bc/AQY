...
public fun close_and_withdraw(self: Kiosk, cap: KioskOwnerCap, ctx: &mut TxContext): Coin<AQY> {
...
public fun profits_mut(self: &mut Kiosk, cap: &KioskOwnerCap): &mut Balance<AQY> {
    assert!(self.has_access(cap), ENotOwner);
    &mut self.profits
}
