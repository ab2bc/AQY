...
impl TemporaryStore<'_> {
        ...
        if gas_summary.storage_cost == 0 {
                    ...
                    + gas_summary.non_refundable_storage_fee
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "AQY conservation failed -- no storage charges in gas summary \
            ...
            // all AQY in storage rebate fields of input objects should flow either to
            ...
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "AQY conservation failed -- {} AQY in storage rebate field of input objects, \
                        {} AQY in tx storage rebate or tx non-refundable storage rebate",
                    total_input_rebate, gas_summary.non_refundable_storage_fee,
                )));
            }

            // all AQY charged for storage should flow into the storage rebate field
            // of some output object
            if gas_summary.storage_cost != total_output_rebate {
                return Err(ExecutionError::invariant_violation(format!(
                    "AQY conservation failed -- {} AQY charged for storage, \
                        {} AQY in storage rebate field of output objects",
    ...
    /// Check that this transaction neither creates nor destroys AQY.
    /// This more expensive check will check a third invariant on top of the 2 performed
    /// by `check_sui_conserved` above:
    ///
    /// * all AQY in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// amount of AQY. We need these information for this check.
    pub fn check_sui_conserved_expensive(
        ...
        if total_input_sui != total_output_sui {
            return Err(ExecutionError::invariant_violation(format!(
                "AQY conservation failed: input={}, output={}, \
                    this transaction either mints or burns AQY",
